...
The anchored floating editor is required and complete. Its primary desktop surface is a compact white note card, approximately 360-380 px wide, with a restrained shadow, dark readable text, and no nested form-card treatment. The top toolbar exposes bold, italic, underline, ordered list, unordered list, an image icon, and close. Reopening an existing note restores every saved text block into the primary textarea in original order so the learner can edit it directly; a separate read-only block card must never be the only copy of saved text. Ordered and unordered controls use stable familiar list glyphs and operate on complete selected lines only. With no selection they affect only the caret line; clicking the active list type again removes that list type; switching list types changes only the selected/current lines. All unselected text and ordering remain byte-for-byte intact apart from necessary range-offset changes. The editor uses a reversible plain-text representation and a safe semantic preview, never persisted arbitrary HTML or `contenteditable`. Saved and legacy list blocks render as semantic `ol`/`ul` content with visible decimal/disc markers even when the lesson shell resets list styles. The body is a quiet, clearly bounded writing surface; the footer exposes delete, the note-surface color/options dot, and a high-contrast save action. It opens from selected text or an existing note icon, stays positioned near the anchor when possible, and supports writing text, pasting text, copying text, uploading images from the image icon, pasting clipboard images directly into the writing surface, copying images, adding captions/alt text, choosing default white/pastel note surfaces, choosing custom `#RRGGBB` note surfaces, saving, canceling, deleting, and export-first clear flows. Keep block type, question tagging, image alt/decorative controls, and color presets in a compact secondary options popover instead of displaying the old full form by default.
...
Accept images from paste, drag/drop, or file input. The image toolbar icon must open the native file picker, and a clipboard image pasted while the note writing surface is focused must enter the same sanitized preview/save pipeline without disturbing ordinary text paste. Allow PNG, JPEG, and WebP only. Reject SVG, GIF, HTML, remote URLs, files above 5 MiB, and decoded images above 12 MP. Re-encode through canvas, strip metadata, and scale the longest edge to at most 1920 px. Require alt text or an explicit decorative flag. Every pending or saved note image shown in the editor, anchored preview, or notes manager is a keyboard-accessible zoom trigger. Open it in a single modal lightbox that contains the full image within the viewport and closes by its close control, backdrop click, or Escape while restoring focus to the trigger. Copying an image back to the clipboard should use the sanitized stored blob when the browser permits it; otherwise provide a clear save/export fallback.
...
- runtime v2 metadata/API, v1 data/package migration, three underline styles, default plus custom `#RRGGBB` mark colors, mark toolbar, compact white B/I/U/list/image note card, saved-note text re-editing, selection-local semantic lists with stable toolbar glyphs, file-picker and clipboard image flows, click/keyboard image lightbox, floating notes manager, status, import/export, and orphan UI
