...
9. Anti-pattern blacklist
   ...
   - Do not remove underlines, highlights, the compact white B/I/U/list/image note card, direct re-editing of saved text, selection-local semantic ordered/unordered lists with stable icons, one-click image upload, direct clipboard image paste, click-to-zoom note images, anchored floating text/image notes, note-icon expand/collapse, anchored source jumps, honest storage status, custom `#RRGGBB` color controls, or portable exports in compact mode.
...
Expected: keep the complete canonical annotation runtime v2, including highlights, all three underline styles, default and custom `#RRGGBB` colors through a compact dropdown, the compact white B/I/U/list/image note card, direct re-editing of saved text, ordered/unordered lists that affect only selected complete lines or the caret line and preserve surrounding order, image upload and direct clipboard image paste, click/keyboard image enlargement, anchored floating text/image notes, note-icon expand/collapse, source jumps, autosave status, `.learnmap` import/export with v1 compatibility, and Markdown export. Compact changes teaching density, not the interaction floor; report content, runtime, and total bytes, but do not fail solely because lesson content exceeds a default target.
